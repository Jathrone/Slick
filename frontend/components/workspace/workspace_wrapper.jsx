...
class WorkspaceWrapper extends React.Component {
    ...
    render() {
        ...
        if (channels.length > 0) {
            redirectComponent.push(<Redirect key={"top-priority-redirect"} exact path={"/"} key={channels[0].id} to={`/channels/${channels[0].id}`}/>);
            redirectComponent.push(< Redirect key={"secondary-priority-redirect"} to = { "/channels/page-not-found"} />);
        }
